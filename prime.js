...
// Exemplo de uso:
const n = Math.pow(5, 7); // Limite superior para encontrar números primos
...
function mAbc(_r = 1, a = "", b = "", c = "", d = "", e = "", f = 1) {
    ...
    let r = `${_r == 1 ? "0" : _r}${testAb(a, 0).r}0${testAb(b, 1).r}0${testAb(c, 2).r}0${testAb(d, 3).r}0${testAb(e, 4).r}0`
    if (!f) return r
...
function dataToList(data) {
    ...
    //console.log(list)
    //console.log(
    let res = list.map(i => "0" + i.join(0) + "0").map(i => qua(i)).map(i => [i.C, i.S, i.M, i.K, i.A]).flat(Infinity)
    //  )
    return res
}
...
const primosEncontrados2 = primosEncontrados.map(p => {
    ...
    console.log(JSON.stringify(result, null, 4), list, list.filter(i=>i).length, list.filter(i=>i).reduce((a, b) => a + b, 0), p)
});
console.log('foi')
